...
#need to implement one hot encoding
dummy_data <- model.matrix(~ borough + room_type - 1, data = train_data)
train_data_encoded <- cbind(train_data, dummy_data)


dummy_test <- model.matrix(~ borough + room_type - 1, data = test_data)
test_data_encoded <- cbind(test_data, dummy_test)

#drop the original categorical variables to prevent duplciate entries
train_data_encoded <- train_data_encoded[, !(names(train_data_encoded) %in% c("room_type", "borough", "neighbourhood_group", "neighbourhood"))]

test_data_encoded <- test_data_encoded[, !(names(train_data_encoded) %in% c("room_type", "borough", "neighbourhood_group", "neighbourhood"))]

#linear regression model 
lm_model <- lm(price ~ borough + room_type + minimum_nights + number_of_reviews + reviews_per_month + availability_365, data = train_data)
