...
# Explore the data 
print(str(df)) #offers a compact preview of the data
print(head(df))
```
Observation:
...
```{r}
print(max(df$minimum_nights)) #checking for information 
print(mean(df$minimum_nights)) #comparative measure of minimum nights
...
#we see a heavily skewed right dist. so we want to remove these outer values
```
...
# # Convert categorical variables to factors through label encoding, works for decision
# # trees but will need to adjust for the linear regression model
...
```{r}
# df_encoded <- df_encoded %>% select(-id,-name,-host_id,-host_name,-room_type, -neighbourhood_group,-neighbourhood,-last_review)
...
train_index <- createDataPartition(df_encoded$log_price, p = 0.8, list = FALSE)
...
# Need to implement one-hot encoding for categorical variables
...
set.seed(123)
tune_grid <- expand.grid(mtry = c(3, 5, 7))  # Test different values of mtry
rf_tuned <- train(
  log_price ~ .,
  ...
  tuneGrid = tune_grid,
  ntree = 300  # Increase the number of trees for better performance
)
...
```{r}
print(colnames(train_data_encoded))
print(colnames(test_data_encoded))
```
...
# Prepare x (predictors) and y (response variable) for training
x_train <- train_data_encoded[, !(names(train_data_encoded) %in% c("price"))]  # Remove the target column
y_train <- train_data_encoded$price  # Response variable

# Prepare x (predictors) for testing
x_test <- test_data_encoded[, !(names(test_data_encoded) %in% c("price"))]  # Remove the target column
y_test <- test_data_encoded$price  # Actual prices for testing

# Fit the Random Forest model using x and y
rf_model <- randomForest(x = x_train, y = y_train, ntree = 100, importance = TRUE, na.action = na.omit)

# Predict on test data
rf_predictions <- predict(rf_model, newdata = x_test)

# Calculate mean absolute error
test_mae_rf <- mean(abs(rf_predictions - y_test))
print(paste("Mean Absolute Error:", test_mae_rf))

# Calculate RMSE
rmse_rf <- sqrt(mean((rf_predictions - y_test)^2))
print(paste("Root Mean Squared Error:", rmse_rf))
...
#rf_model <- randomForest(price ~., data = train_data_encoded, ntree = 1, importance = TRUE, na.action = #na.omit)
...
#Calculate mean absolute error
#test_mae_rf <- mean(abs(rf_predictions - test_data_encoded$price))

# Calculate RMSE
#rmse_rf <- sqrt(mean((rf_predictions - test_data_encoded$price)^2))
...
rf_cv_model <- train(x = x_train,
                     y = y_train,
                     method = "rf",  # Random forest
                     trControl = train_control,
                     tuneGrid = expand.grid(mtry = seq(1, ncol(x_train), by = 2)),  # Hyperparameter tuning for mtry
...
print(paste("Best mtry:", rf_cv_model$bestTune))

# Predict on test data using the best model
rf_cv_predictions <- predict(rf_cv_model, newdata = x_test)

# Calculate mean absolute error
test_mae_rf_cv <- mean(abs(rf_cv_predictions - y_test))
print(paste("Mean Absolute Error (CV):", test_mae_rf_cv))

# Calculate RMSE
rmse_rf_cv <- sqrt(mean((rf_cv_predictions - y_test)^2))
print(paste("Root Mean Squared Error (CV):", rmse_rf_cv))
...
#set.seed(123)
#rf_cv_model <- train(price ~ borough_num + room_type_num + minimum_nights + number_of_reviews + reviews_per_month + calculated_host_listings_count + availability_365, data = train_data, method = "rf", trControl = train_control, ntree=100)
# 
# # Model performance
#rf_cv_model$results
