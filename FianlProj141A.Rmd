...
lm_model <- lm(price ~. - id - host_id - name - host_name - neighbourhood - last_review - borough_num
               - room_type_num - latitude - longitude, data = train_data)
...
rf_model <- randomForest(price ~ borough_num + room_type_num + minimum_nights + number_of_reviews + reviews_per_month + calculated_host_listings_count + availability_365, data = train_data, ntree = 100, importance = TRUE, na.action = na.omit)

# Predict on test data
rf_predictions <- predict(rf_model, newdata = test_data)

#Calculate mean absolute error
test_mae_rf <- mean(abs(rf_predictions - test_data$price))
...
# set.seed(123)
# rf_cv_model <- train(price ~ borough_num + room_type_num + minimum_nights + number_of_reviews + reviews_per_month + calculated_host_listings_count + availability_365, data = train_data, method = "rf", trControl = train_control,
#                      ntree=100)
# 
# # Model performance
# rf_cv_model$results
